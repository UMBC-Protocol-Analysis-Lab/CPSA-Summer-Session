...
\begin{frame}\frametitle{Number Fields}
\pause
Let \(p\) be a prime, and \(g \in \{1, \dots, p-1\}\).

\pause
\begin{definition}
\(g\) is called a \emph{generator} for the finite field \(\Z/p\Z\) (the numbers \(\mod p\)) if 
there exists some \(f\) such that
\[
f(\{g^1,\dots,g^{(p-1)}\}) = \{1,\dots,p-1\}
\]
That is, \(f\) is onto, or surjective.
\end{definition}
\pause
...
\begin{frame}\frametitle{Some CPSA hints}
\begin{itemize}
\item Need correct origination assumptions
\begin{itemize}
\item ‘a’ and ‘b’ need to be pen-non-orig
...
\pause
\begin{center}
{\Huge Modeling from a spec in CPSA}
\end{center}
\pause
Modeling a real world protocol takes some practice, so let's try!

You will have a full week to model SCEP from SecureDNA.

A writeup of the spec can be found in the homework folder.

\emph{Please} ask for help if you need it, SCEP is complicated and we are here to help!
\end{frame}
