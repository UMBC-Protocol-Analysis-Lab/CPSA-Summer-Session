...
\title{Session 5: Diffie-Hellman and Specifications}
...
\begin{frame}\frametitle{Math Review}
\begin{enumerate}
\item[Pre] Let \(g,a,b \in \N\) (they are all natural numbers)
\pause
\item \(g^a = \underbrace{g \times g \times g \times \dots \times g}_{a \text{ times}} \)
\pause
\item \(g^a \times g^b = g^{a+b}\).
\pause
\item \((g^a)^b = \overbrace{g^a \times \dots \times g^a}^{b \text{ times}} = g^{ab} = g^{ba} = \overbrace{g^b \times \dots \times g^b}^{a \text{ times}} = (g^b)^a\)
\pause
\item[Note] \(g^{a^b} \neq g^{b^a}\) in general (power tower vs.\ commutativity of multiplying exponents).
\end{enumerate}
\end{frame}
...
\(g\) is a \emph{generator} for some \(p\) if:
\[
(\{g^1,\dots,g^{(p-1)}\} \mod p) = \{1,\dots,p-1\}
\]
That is, \(g\) is a generator if the above function is \emph{surjective}.

The set $\{ 1, \dots , p-1\}$ equipped with multiplication is known as \(\Z_p^\times\)
\end{definition}
\pause
...
\begin{frame}\frametitle{Exponents}
    \pause
    Recall the exponent property (2 from before)
    \[
    x^a \cdot x^b = x^{a + b}
    \]
    \pause
    ...
    \pause
    We call these exponent terms \(a\) and \(b\) random exponents, with sort \texttt{rndx}.

\end{frame}

\begin{frame}\frametitle{DH Exchange}
\begin{figure}
    \centering
    \includegraphics[height=\textheight]{slide-images/dh-exchange}
...
\begin{itemize}
\item `a' and `b' (of sort \texttt{rndx}) need to be uniq-gen
...
\begin{frame}\frametitle{Diffie-Hellman Fix}
\begin{tikzpicture}
    \node at (0,8) {Alice};
    \node at (8,8) {Bob};
    \node[shape=circle,draw=blue, fill=blue] at (1,0) {};
    ...
    \node at (4,5.25) {\(\enc(alice, bob, A, privk(alice))\)};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    \node[shape=circle,draw=black, fill=black] (3) at (8,4) {};
    \node[shape=circle,draw=blue, fill=blue] (4) at (0,4) {};
    
    \path [-,thick](3) edge (2);
    \path [saiarrow](3) edge (4);
    \path [-,thick](1) edge (4);
    
    \node at (4,4.22) {\(\enc(alice, bob, B, privk(bob))\)};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    \node[shape=circle,draw=blue, fill=blue] (5) at (8,3) {};
    \node[shape=circle,draw=black, fill=black] (6) at (0,3) {};

    \path [-,thick](5) edge (3);
    \path [saiarrow](6) edge (5);
    \path [-,thick](6) edge (4);

    \node at (4,3.22) {\(\enc(\text{``hello''}, B^a \mod p)\)};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    \node[shape=circle,draw=black, fill=black] (7) at (8,2) {};
    \node[shape=circle,draw=blue, fill=blue] (8) at (0,2) {};

    \path [-,thick](6) edge (8);
    \path [saiarrow](7) edge (8);
    \path [-,thick](5) edge (7);

    \node at (4,2.22) {\(\enc(\text{``hello''}, A^b \mod p)\)};
...
\pause
\begin{center}
{\Huge Fix your meeting protocol}
\end{center}

Now that you have more experience in CPSA, see if you can fix your meeting protocol.

You are going to model your meeting protocol in CPSA, see what flaws it has, and then try to fix it using some of the techniques we have discussed.
...
\begin{figure}
    \centering
    \includegraphics[height=0.88\textheight]{qr-codes/Survey5.png}
\end{figure}
\end{frame}
\end{document}