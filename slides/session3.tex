...
\pause
\item Missing/extra strands
\item[]
\pause
\item What do we notice in the NS shapes from last time?
\end{itemize}
\end{frame}

\begin{frame}\frametitle{NS shapes from last time}
\begin{columns}
\column{0.4\textwidth}
\begin{figure}
\includegraphics[height=0.7\textheight]{slide-images/ns-alice}
\caption{NS from Alice's perspective}
\end{figure}

\column{0.4\textwidth}
\begin{figure}
\includegraphics[height=0.7\textheight]{slide-images/ns-bob}
\caption{NS from Bob's perspective}
\end{figure}
\end{columns}
\end{frame}
...
\begin{tikzpicture}
    ...
    \node at (5,8) {Bob-0};
    \node at (10,8) {Bob};
    \node[shape=circle,draw=blue, fill=blue] at (0,0) {};
    \node at (1.25,0) {= Reception};
    \node[shape=circle,draw=black, fill=black] at (3,0) {};
    \node at (4.5,0) {= Transmission};
    \node[shape=circle, draw=black, fill=purple] at (6.25,0) {};
    \node at (8.5,0) {= Forward (nonstandard)};

    \node[shape=circle,draw=black, fill=black] (1) at (0,7) {};
    \node[shape=circle,draw=blue, fill=blue] (2) at (10,7) {};
    \node[shape=circle,draw=black, fill=purple] (1m) at (5,7) {};
    
    \path [saiarrow](1) edge (1m);
    \path [saiarrow](1m) edge (2);
    
    \node at (2.5,7.25) {\(enc(N_a, Alice, pubk_{Bob-0})\)};
    \node at (7.5,7.25) {\(enc(N_a, Alice, \isred{pubk_{Bob}})\)};
    
    \node[shape=circle,draw=black, fill=black] (3) at (10,5.5) {};
    \node[shape=circle,draw=blue, fill=blue] (4) at (0,5.5) {};
    \node[shape=circle,draw=black, fill=purple] (3m) at (5,5.5) {};
    ...
    \node[shape=circle,draw=blue, fill=black] (5) at (0,4) {};
    \node[shape=circle,draw=blue, fill=blue] (6) at (10,4) {};
    \node[shape=circle,draw=black, fill=purple] (5m) at (5,4) {};
    ...
    \node at (2.5,4.2) {\(enc(N_b,pubk_{Bob-0})\)};
    \node at (7.5,4.2) {\(enc(N_b,\isred{pubk_{Bob}})\)};
...
Two of the protocols are already done, you just have to run \texttt{make} so CPSA can make shapes, and then analyze them.
...
Due at 6:00pm on Thursday the 23rd
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{qr-codes/Survey3QRCode.png}
